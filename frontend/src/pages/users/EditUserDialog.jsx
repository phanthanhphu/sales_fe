...
export default function EditUserDialog({ open, onClose, onUpdate, user, disabled = false }) {
  ...
  const save = async () => {
    if (!user?.id) return showNotice('User ID is missing.');
    ...
    try {
      ...
      if (!response.ok) throw new Error(body?.message || raw || `Unable to update user (${response.status}).`);
      onUpdate?.({ ...user, ...(body?.data || body), ...form }); onClose?.();
    } catch (error) { showNotice(error.message || 'Unable to update user.'); }
  ...
  return <>
    <Dialog open={open} onClose={locked ? undefined : onClose} fullScreen={fullScreen} maxWidth="md" fullWidth PaperProps={{ sx: stableDialogPaperSx(fullScreen) }}>
      <DialogTitle sx={stableDialogTitleSx}><Typography component="div" sx={{ fontSize: '1.15rem', fontWeight: 750 }}>Edit User</Typography><IconButton aria-label="Close" onClick={onClose} disabled={locked} sx={stableCloseButtonSx}><CloseRoundedIcon /></IconButton></DialogTitle>
