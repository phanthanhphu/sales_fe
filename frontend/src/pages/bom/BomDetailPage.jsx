      ...
      <DialogContent dividers>
        ...
        <Box sx={{ mt: 2, border: '1px solid #e5e7eb', borderRadius: 1.5, p: 1.25 }}>
          <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ xs: 'flex-start', sm: 'center' }} spacing={0.75}>
            <Box>
              <Typography sx={{ fontWeight: 700, fontSize: '0.83rem' }}>Applicable Product Colors</Typography>
    ...
    <Box sx={{ p: { xs: 0.75, sm: 1, md: 1.25 } }}>
          ...
          <Box sx={{ minWidth: 0, flex: 1 }}>
            <Stack direction="row" spacing={0.7} alignItems="center" flexWrap="wrap" useFlexGap>
              <Button
                ...
                size="small"
                sx={{ px: 0.15, minHeight: 24, color: '#49657f', fontSize: '.68rem', textTransform: 'none' }}
              >
                ← Order
              </Button>
              <Typography
      ...
      <Paper id="bom-core-materials" elevation={0} sx={{ scrollMarginTop: 88, border: '1px solid #e5e7eb', borderRadius: 1.7, overflow: 'hidden', mb: 1 }}>
        <SectionHeader
          ...
          actions={(
            <Tooltip title={!canWrite ? writeBlockedMessage : 'Add Material'}>
              <span><Button startIcon={<Add />} variant="contained" size="small" disabled={!canWrite} onClick={() => setLineCtx({ record: null, packingId: '' })} sx={{ backgroundColor: '#103B5C' }}>Add Material</Button></span>
            </Tooltip>
          )}
        />
          ...
          >
            ...
            >
              <Stack direction="row" spacing={0.65} alignItems="center" sx={{ width: 1, pr: 0.5 }}>
                ...
                <Chip size="small" label={linkedProductColors.length ? `${linkedProductColors.length} Colors` : 'All Colors'} variant="outlined" sx={{ height: 21, fontSize: '.6rem' }} />
                <Box sx={{ flex: 1 }} />
            ...
            <AccordionDetails sx={{ p: 0.8 }}>
              <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ xs: 'stretch', sm: 'center' }} spacing={0.6} sx={{ mb: 0.65 }}>
                <Stack direction="row" flexWrap="wrap" gap={0.35}>
                  {linkedProductColors.length
                    ? linkedProductColors.map((item) => <Chip key={item.id} size="small" label={productColorLabel(item)} sx={{ height: 21, fontSize: '.6rem' }} />)
                    : <Chip size="small" variant="outlined" label="All Product Colors" sx={{ height: 21, fontSize: '.6rem' }} />}
                </Stack>
                <Tooltip title={!canWrite ? writeBlockedMessage : 'Add Packing Line'}><span><Button size="small" startIcon={<Add />} disabled={!canWrite} onClick={() => setLineCtx({ record: null, packingId: packing.id })} sx={{ textTransform: 'none', whiteSpace: 'nowrap', minHeight: 29 }}>
                  Add Line
                </Button></span></Tooltip>
              </Stack>
