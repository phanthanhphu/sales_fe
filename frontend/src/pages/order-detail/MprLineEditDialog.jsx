...
export default function MprLineEditDialog({ open, line, productColorMasters = [], saving, onClose, onSave }) {
  ...
  return (
    <Dialog open={open} onClose={saving ? undefined : onClose} fullWidth maxWidth="md">
      <DialogTitle sx={{ fontWeight: 750, color: '#103B5C' }}>Edit MPR Item</DialogTitle>
      <DialogContent dividers>
        ...
        <Typography sx={{ fontSize: '.84rem', fontWeight: 750, mb: .75 }}>Style And BOM Information</Typography>
        ...
        <Divider sx={{ my: 2 }} />
        <Typography sx={{ fontSize: '.84rem', fontWeight: 750, mb: .75 }}>Quantity Calculation</Typography>
        ...
        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1} sx={{ mt: 1.25 }}>
          <Box sx={{ flex: 1, p: 1.25, border: '1px solid #e5e7eb', borderRadius: 1.5 }}>
            ...
            <Typography sx={{ fontWeight: 750 }}>{displayNumber(calculated.totalYield)}</Typography>
          </Box>
          <Box sx={{ flex: 1, p: 1.25, border: '1px solid #e5e7eb', borderRadius: 1.5 }}>
            <Typography sx={{ fontSize: '.72rem', color: 'text.secondary' }}>MAT REQUIRED Q&apos;TY</Typography>
            <Typography sx={{ fontWeight: 750 }}>{displayNumber(calculated.required)}</Typography>
          </Box>
          <Box sx={{ flex: 1, p: 1.25, border: '1px solid #e5e7eb', borderRadius: 1.5 }}>
            <Typography sx={{ fontSize: '.72rem', color: 'text.secondary' }}>MAT SAMPLE Q&apos;TY</Typography>
            <Typography sx={{ fontWeight: 750 }}>{displayNumber(calculated.matSample)}</Typography>
          </Box>
          <Box sx={{ flex: 1, p: 1.25, border: '1px solid #e5e7eb', borderRadius: 1.5 }}>
            <Typography sx={{ fontSize: '.72rem', color: 'text.secondary' }}>SAP STOCK QTY</Typography>
            <Typography sx={{ fontWeight: 750 }}>{displayNumber(calculated.sapStock)}</Typography>
          </Box>
          <Box sx={{ flex: 1, p: 1.25, border: '1px solid #e5e7eb', borderRadius: 1.5 }}>
            <Typography sx={{ fontSize: '.72rem', color: 'text.secondary' }}>PURCHASE QTY</Typography>
            <Typography sx={{ fontWeight: 750 }}>{displayNumber(calculated.purchase)}</Typography>
          </Box>
        </Stack>

        <Divider sx={{ my: 2 }} />
        <Typography sx={{ fontSize: '.84rem', fontWeight: 750, mb: .75 }}>Commercial And Supplier Information</Typography>
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, minmax(0, 1fr))' }, gap: 1.25 }}>
