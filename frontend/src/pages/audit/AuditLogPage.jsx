...
export default function AuditLogPage() {
  ...
  return (
    <Box>
      <Paper elevation={0} sx={{ p: 0.85, border: '1px solid #e5e7eb', borderRadius: 1.7, mb: 0.8 }}>
        ...
        <Stack direction="row" spacing={0.7} justifyContent="flex-end" sx={{ mt: 0.75 }}>
          <Button variant="outlined" startIcon={<Refresh fontSize="small" />} onClick={load} disabled={loading} sx={{ textTransform: 'none', height: 34 }}>Refresh</Button>
          <Button variant="outlined" startIcon={<RestartAlt fontSize="small" />} onClick={reset} disabled={loading} sx={{ textTransform: 'none', height: 34 }}>Reset</Button>
          <Button variant="contained" startIcon={<Search fontSize="small" />} onClick={apply} disabled={loading} sx={{ textTransform: 'none', height: 34, backgroundColor: '#103B5C' }}>Search</Button>
        </Stack>
      </Paper>
      ...
      <Paper elevation={0} sx={{ border: '1px solid #e5e7eb', borderRadius: 2, overflow: 'hidden' }}>
        <TableContainer>
          <Table size="small">
            <TableHead>
              <TableRow>
                {['No.', 'Time', 'User', 'Module', 'Action', 'Result', 'Resource / File', 'Duration', 'Details'].map((heading) => (
                  <TableCell key={heading} sx={{ fontWeight: 750, backgroundColor: '#f8fafc', whiteSpace: 'nowrap' }}>{heading}</TableCell>
                ))}
              </TableRow>
            </TableHead>
            <TableBody>
              {loading && <TableRow><TableCell colSpan={9}>Loading audit logs...</TableCell></TableRow>}
              {!loading && rows.length === 0 && <TableRow><TableCell colSpan={9} align="center" sx={{ py: 5, color: 'text.secondary' }}>No audit logs match the current filters.</TableCell></TableRow>}
