...
export default function EditDepartmentDialog({ open, onClose, department, disabled = false }) {
  ...
  useEffect(() => {
    ...
    setDepartmentName(department?.departmentName || department?.name || '');
    setSaving(false);
    setErrors({});
  ...
  const save = async () => {
    if (!department?.id) {
      setNotice({ open: true, message: 'Department ID is missing.', severity: 'error' });
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        let message = `Unable to update department (${response.status}).`;
        try {
      ...
      setNotice({ open: true, message: error.message || 'Unable to update department.', severity: 'error' });
    } finally {
      setSaving(false);
    }
  };
