...
export default function BomCreateDialog({ open, saving, onClose, onSave }) {
  ...
  return <Dialog open={open} onClose={saving ? undefined : onClose} fullWidth maxWidth="sm">
    <DialogTitle sx={{ fontWeight: 900, color: '#103B5C' }}>Add BOM<IconButton onClick={onClose} disabled={saving} sx={{ position: 'absolute', top: 12, right: 12 }}><Close /></IconButton><Typography sx={{ fontSize: '0.8rem', color: 'text.secondary', fontWeight: 400 }}>Create a blank BOM. You can add packings, materials, and images afterward.</Typography></DialogTitle>
