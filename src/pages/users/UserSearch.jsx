  ...
  return (
    <Paper elevation={0} sx={{ p: 0.85, mb: 0.8, borderRadius: 1.7, border: '1px solid #e5e7eb', backgroundColor: '#fff', overflow: 'hidden' }}>
      <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.75, alignItems: 'center', width: '100%' }}>
        ...
        <FormControl fullWidth disabled={disabled} size="small" sx={fieldSx}><InputLabel>System Access</InputLabel><Select value={searchAccessPermission || ''} label="System Access" onChange={(event) => { resetPage(); setSearchAccessPermission?.(event.target.value); }}><MenuItem value="">All access</MenuItem><MenuItem value="BOM">BOM</MenuItem><MenuItem value="SALES">Sales</MenuItem><MenuItem value="REOPEN_COMPLETED_MPR">Reopen Completed MPR</MenuItem><MenuItem value="VIEW_SYSTEM">View System</MenuItem></Select></FormControl>
