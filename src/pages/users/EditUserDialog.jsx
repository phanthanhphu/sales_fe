...
export default function EditUserDialog({ open, onClose, onUpdate, user, disabled = false }) {
  ...
  return <>
    <Dialog open={open} onClose={locked ? undefined : onClose} fullScreen={fullScreen} maxWidth="md" fullWidth PaperProps={{ sx: stableDialogPaperSx(fullScreen) }}>
      <DialogTitle sx={stableDialogTitleSx}><Typography component="div" sx={{ fontSize: '1.15rem', fontWeight: 900 }}>Edit User</Typography><IconButton aria-label="Close" onClick={onClose} disabled={locked} sx={stableCloseButtonSx}><CloseRoundedIcon /></IconButton></DialogTitle>
