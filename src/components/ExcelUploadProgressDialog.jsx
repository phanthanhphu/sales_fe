  ...
  return (
    ...
    >
      <DialogTitle sx={{ px: 2.5, py: 1.7, pr: 6, borderBottom: '1px solid #e4edf5', backgroundColor: 'rgba(255,255,255,.94)' }}>
        <Typography sx={{ fontWeight: 950, color: '#103B5C', fontSize: '1.02rem' }}>{title}</Typography>
      ...
      <DialogActions sx={{ px: 2.4, py: 1.55, gap: 0.7, borderTop: '1px solid #e4edf5', backgroundColor: '#fff' }}>
        {failed && onRetry && (
          <Button onClick={onRetry} variant="outlined" sx={{ textTransform: 'none', fontWeight: 800 }}>
            Retry
          </Button>
        )}
        <Button
